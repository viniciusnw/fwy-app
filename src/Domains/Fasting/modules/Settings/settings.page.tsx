...
class Settings extends React.Component<RoutePropsType & ReduxPropsType & PagePropsType, any> {
  ...
  render() {
    ...
    const social = ['Follow on Instagram', 'Follow on Facebook', 'Website', 'WhatsApp']
    ...
    return (
      <View style={{ flex: 1, alignItems: 'center', marginHorizontal: 20 }}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} showsHorizontalScrollIndicator={false} style={{ width: '100%' }}>
          ...
          {prefs.map((item, idx) => (
            <TouchableOpacity key={idx} style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 10, paddingVertical: 20 }}>
              <StyledText>{item}</StyledText>
              <Icon icon={'right'} color={'#FFF'} size={28} />
              {idx + 1 != prefs.length
                && <View style={{ bottom: 0, height: 1, right: 0, left: 0, position: 'absolute', backgroundColor: 'rgba(255, 255, 255, .4)' }} />}
            </TouchableOpacity>
          ))}
        ...
        <View style={{ paddingVertical: 60 }}>
          ...
          <Text style={{ color: '#FFF', textAlign: 'center' }}>
            ...
            <Text style={{ fontSize: 14 }}>Design by <Text style={{ fontWeight: 'bold' }}>eview</Text> design</Text>
          </Text>
        </View>
      </View>
    );
